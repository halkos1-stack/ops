...
async function buildCanonicalConditionsPayload(propertyId: string) {
  ...
  return {
    property,
    nextBooking: refreshed.canonicalTruth.nextBooking,
    snapshot: refreshed.canonicalTruth.snapshot,
    readiness: refreshed.canonicalTruth.readiness,
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const createdItem =
      payload.snapshot.conditions.find(
        (condition) => condition.id === createdCondition.id
      ) ?? null
