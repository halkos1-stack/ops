...
Το OPS είναι πολυοργανωσιακό SaaS λειτουργικής διαχείρισης ακινήτων με πυρήνα την **επιχειρησιακή ετοιμότητα ακινήτου πριν το αμέσως επόμενο check-in από σήμερα**.
...
αλλά **δεν** επηρεάζουν readiness για το επόμενο check-in, εκτός αν αφήνουν ανοιχτό θέμα που συνεχίζει να επηρεάζει το active target.
...
### 11. Η σελίδα Ακίνητα δείχνει την εικόνα για το active readiness target

Η σελίδα **Ακίνητα** αποτυπώνει για κάθε ακίνητο την κατάσταση **για την επόμενη άφιξη**.

Δεν αποτυπώνει γενικά όλη τη λειτουργική δραστηριότητα του ακινήτου.
...
- hover επεξήγηση της κατάστασης και των ενεργειών που χρειάζονται

Οι in-stay εργασίες όσο το ακίνητο έχει κόσμο **δεν** επηρεάζουν την κύρια ένδειξη της σελίδας Ακίνητα, εκτός αν αφήνουν ανοιχτό θέμα που συνεχίζει να επηρεάζει την επόμενη άφιξη.

### 12. Η σελίδα Ακίνητο είναι το πλήρες κέντρο διαχείρισης
...
- ορίζει τον προεπιλεγμένο συνεργάτη
- δημιουργεί εργασίες όποτε θέλει
- βλέπει την πλήρη λειτουργική εικόνα του ακινήτου
- διαχειρίζεται αναλώσιμα, ζημιές, βλάβες και ανοιχτά θέματα

### 13. Το περιβάλλον των σελίδων Ακίνητα και Ακίνητο δεν αλλάζει χωρίς ρητή εντολή
...
δεν αλλάζει χωρίς ρητή εντολή.

Δεν επιτρέπονται αυθαίρετες αλλαγές διάταξης, λογικής παρουσίασης ή ιεραρχίας πληροφορίας χωρίς ρητή απόφαση.
...
Επίσης ο διαχειριστής πρέπει να μπορεί να δημιουργεί κράτηση και χειροκίνητα.

### Ροή εργασίας

`Task -> assignment -> partner portal -> execution lists -> submit -> conditions / readiness impact`

Ο διαχειριστής μπορεί να δημιουργεί εργασία οποτεδήποτε, είτε το ακίνητο έχει κόσμο είτε όχι.
...
`Partner enters property -> QR scan -> work session open -> execution -> submit -> session close -> duration / proof`
...
Ο διαχειριστής μέσα από τη σελίδα του ακινήτου:

- δημιουργεί τις λίστες για το συγκεκριμένο ακίνητο
- ορίζει τον προεπιλεγμένο συνεργάτη
...
Ο διαχειριστής πρέπει να μπορεί να ορίζει αριθμητικά όρια για:

- έλλειψη
- μέτρια κατάσταση
- πλήρη κατάσταση

Ο συνεργάτης πρέπει να μπορεί να δηλώνει:

- σε ποιο επίπεδο βρίσκονται τα αναλώσιμα
...
#### Κανόνας στατιστικών
Το σύστημα πρέπει να υποστηρίζει dedicated στατιστική εικόνα κατανάλωσης αναλωσίμων για χρονικό διάστημα από - έως.
...
Οι ζημιές και οι βλάβες πρέπει να μπορούν να ανεβαίνουν από φόρμα μέσα στο partner portal.

Ο διαχειριστής πρέπει να μπορεί μέσω του συστήματος:

- να ενημερώνει όποιον συνεργάτη θέλει
- να οργανώνει τον χειρισμό του θέματος
- να κλείνει χειροκίνητα το θέμα όταν αποκατασταθεί
- να δέχεται αποκατάσταση και από υποβολή φόρμας
...
#### Κανόνας ιστορικού
Ό,τι έχει κλείσει μένει στο ιστορικό.
Ό,τι δεν έχει κλείσει παραμένει ανοιχτό.
...
Κάθε ακίνητο πρέπει να έχει το δικό του QR.
...
- το UI στις σελίδες Ακίνητα και Ακίνητο δεν αλλάζει χωρίς ρητή εντολή
